...
- ``model_dir`` (*str*, optional): existing directory for the best checkpoint. A nonexistent path raises ``ValueError``.
...
- ``text_input_dim`` (*int*, optional): vector input width. The default is 4096.
...
- ``video_in_channels`` (*int*, optional): video input channels. The default is 1.
...
- ``fit(R, Y, W, mask, C=None, valid_perc=0.2, plot_loss=True, *, R_video=None, epoch_callback=None)`` trains with early stopping and returns the best validation loss as a float.
- ``validate_step()`` evaluates the current validation loader and returns scalar MSE.
- ``predict(R, W, mask, C=None, return_rep=False, *, R_video=None)`` returns outcome predictions with shape ``[N, 1]``. With ``return_rep=True``, it returns ``(predictions, h, h_flat)``, where ``h`` is ``[N, T, architecture_z[-1]]`` and ``h_flat`` is ``[N, T * architecture_z[-1]]``.

The treatment ``W`` and mask must both have shape ``[N, T]``. ``R`` accepts ``[N, T, F]`` or ``[F, N, T]``. In multimodal mode, ``R_video`` accepts ``[N, T, D, H, W]`` or ``[N, T, C, D, H, W]``.
